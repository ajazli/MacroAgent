...
def _aggregate_today(logs: list) -> dict:
    ...
    for row in logs:
        ...
        if t == "meal":
            ...
            totals["sleep_hours"] = data.get("hours")
        elif t == "energy":
            totals["energy_level"] = data.get("level")
        elif t == "workout":
...
def format_today_summary(
    ...
    sleep_line   = escape(f"{t['sleep_hours']} hrs") if t["sleep_hours"] is not None else escape("—")
    energy_line  = escape(f"{t['energy_level']}/10") if t["energy_level"] is not None else escape("—")

    streak_line = ""
    if streak >= 2:
        streak_line = f"🔥 *{escape(str(streak))}\\-day streak\\!*\n"
    ...
    if streak_line:
        lines.append(streak_line.rstrip())
    lines += [
        escape("━━━━━━━━━━━━━━━"),
...
def format_log_confirmation(log_type: str, data: dict) -> str:
    if log_type == "weight":
        ...
        hours = escape(str(data.get("hours", 0)))
        return f"✅ Sleep logged: *{hours} hours*"
    elif log_type == "energy":
        level = escape(str(data.get("level", 0)))
...
def format_daily_nutrition_summary(meal_rows: list) -> str:
    ...
    def _short_name(desc: str) -> str:
        """Extract a short meal name from a verbose Claude description."""
        name = re.split(r"[,(]| with | — ", desc)[0].strip()
    ...
    for name in sorted(by_user):
        ...
        for cat in _CAT_ORDER:
            ...
            for row in rows_in:
                ...
                pro   += float(d.get("protein", 0) or 0)
                carbs += float(d.get("carbs", 0) or 0)
                fat   += float(d.get("fat", 0) or 0)
                names.append(_short_name(d.get("description", "Meal")))
            total_cal += cal; total_pro += pro
            total_carbs += carbs; total_fat += fat
            emoji = _CAT_EMOJI[cat]
            meal_list = escape(", ".join(names))
            cat_lines.append(f"{emoji} {meal_list} — {escape(str(int(cal)))} kcal")

        lines.append(f"👤 *{escape(name)}*")
        lines.extend(cat_lines)
        lines.append(
            f"📊 *{escape(str(int(total_cal)))} kcal* \\| "
...
def format_check_in_status(schedules: list) -> str:
    ...
    for s in schedules:
        name     = escape(s["name"])
        date_str = escape(s["scheduled_date"].strftime("%d %b %Y"))
        if s.get("completed_at"):
            status = "✅ Done"
        elif s.get("prompted_at"):
            status = "⏳ Prompted"
        else:
            status = "🕐 Pending"
